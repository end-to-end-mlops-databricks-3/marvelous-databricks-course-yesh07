...
from mlops_course.config.config import load_config_from_env
...
def create_spark_session(app_name: str, config: dict | None = None) -> SparkSession:
...
def run_preprocessing(config_path: str | None = None):
    ...
    # Log results
    logger.info(
        f"Preprocessing completed successfully. Result has {result_df.count()} rows and {len(result_df.columns)} columns"
    )
